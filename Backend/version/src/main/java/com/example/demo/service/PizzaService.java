...
public class PizzaService {
	...
	public boolean isValid(Pizza pizza) {
		if(pizza.getPrice()<0) {
			logger.error("error !! pizza value cannot be negative");
	...
	public Pizza addPizza(Pizza pizza) {
		if(!isValid(pizza)) {
			logger.warn("pizza is not valid");
			return null;
		}
		if(pizzaRepository.findByName(pizza.getName())!=null) {
			logger.error("pizza with name "+pizza.getName()+" already exists");
	...
	public String deletePizza(int id) {
		...
		if(pizza==null) {
			logger.warn("No pizza found with id "+ id);
	...
	public void updatePizza(Pizza pizza) {
		...
		if(newpizza==null) {
			logger.warn("No pizza found with id "+ pizza.getId());
